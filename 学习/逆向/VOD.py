...
class Get_data:
    ...
    @get
    def get_html(url2):
        html1 = requests.get(url=url2, headers=Get_data.headers, proxies={'http': 'https://127.0.0.1:7890'})
...
if __name__ == '__main__':
    get_dat = Get_data()
    get_dat.url = 'https://hoho.tv/vod/type/id/4.html'
    sel = get_dat.get_html(get_dat.url)
    title_list = sel.css('div.module-item-titlebox > a::text').getall()
    img_list = ['https://hoho.tv' + i for i in sel.css('div.module-item-pic > img::attr(data-src)').getall()]
    data_1 = dict(zip(title_list, img_list))
    print(data_1)
