...
if __name__ == '__main__':
    ...
    for i in names:
        if not '.py' in i:
            ...
            paths.append(i)
    for j in paths:
        name = path_analysis(source_path + '\\' + j)
        data = []
        for i in name:
            str_2 = i
            datanames = os.listdir(source_path + '\\' + j + '\\' + str_2)
            # 排序有问题,得
            datanames.sort(key=lambda x: int(x[:-4]))
            data.append([source_path + '\\' + j + '\\' + str_2 + '\\', '.jpg', i + '.html', int(datanames[0][0:-4]), len(datanames)])
        ...
        for d in data:
            ...
            strat = d[3]
            finish = d[4]
            with open(j + '\\' + output, 'w', encoding='utf-8') as f:
                f.write(
                    "<html><meta charset='UTF-8'><style>div {margin: auto;width: 900px;}img{width: 95%;} .pic {position: relative;} .pic span {position: absolute;bottom: 0;right: -2px;}</style><body><div>")
                for i in range(strat, finish):
                    ...
                    # f.write("%d" % i)
                f.write(
                    "<button><a href='" + pre + "'>前一章 </a></button> <button><a href='" + next + "'> 下一章</a></button>   </div><script>"
